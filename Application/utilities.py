...
def read_transport_requests(file_path, carrierId, socketio): # path?
    deliveries_df = pd.read_csv(file_path) # eventually clean data first
    print(f"\nAll deliveries: \n")
    print(tabulate(deliveries_df, headers='keys', tablefmt='psql'))
    socketio.emit(carrierId, {'message': tabulate(deliveries_df, headers='keys', tablefmt='psql')})
    return deliveries_df 


def get_requests_below_thresh(df, carrierId, socketio, thresh=100):
    """returns a list of """
    ...
    socketio.emit(carrierId, {'message': tabulate(below_threshold, headers='keys', tablefmt='psql')})
    ...
    socketio.emit(carrierId, {'message': f"Transport request to send: {requests_list}"})
