...
class Auctioneer:
    ...
    """
    def __init__(self, socketio, host=socket.gethostname(), port=12349):
        """
        ...
        self.socketio.emit('auctioneer_log', {'message': "Auctioneer agent is ready"})

    def start_server(self):
        """
        Start the auctioneer server to listen for incoming connections
        """
        self.socketio.emit('auctioneer_log', {'message': "Starting server..."})
            ...
            if self.auction_time is not None:
                for i in range(5):
                    ...
                    if not self.next_round:
                        ...
                        self.socketio.emit('auctioneer_log', {'message': "Auction day closed server restarts tomorrow..."})
                        self.stop_server()
                        break
                    else:
    ...
    def handle_carrier(self, carrier_socket):
        ...
        try:
            ...
            self.socketio.emit('auctioneer_log', {'message': f"Error occure during handling connection with carrier {carrier_socket.getpeername()}. \nError : {e}\n"})
